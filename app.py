...
st.set_page_config(page_title="Detection Cancer Pulmonaire", page_icon="⚕", layout="wide")
...
with col1:
    age = st.slider("Âge", 20, 90, 60)
    sexe = st.selectbox("Sexe", ["Femme", "Homme"])
    presence_nodule = st.selectbox("Présence de nodule", ["Non", "Oui"])
    subtilite = st.slider("Subtilité du nodule", 1, 5, 3)
    taille = st.selectbox("Taille du nodule (px)", [0, 1])

with col2:
    x_nodule = st.slider("X nodule (normalisé)", 0.0, 1.0, 0.5)
    y_nodule = st.slider("Y nodule (normalisé)", 0.0, 1.0, 0.5)
    tabagisme = st.slider("Tabagisme (paquets-années)", 0.0, 50.0, 20.0)
    toux = st.selectbox("Toux chronique", ["Non", "Oui"])
    dyspnee_val = st.selectbox("Dyspnée", ["Non", "Oui"])
...
uploaded_file = st.file_uploader("Charger une radio (JPG/PNG)", type=["jpg", "jpeg", "png"])
if uploaded_file:
...
if st.button("🔍 Lancer la prédiction", type="primary"):
    ...
    risk_labels = ["Faible", "Intermédiaire", "Élevé"]
    risk_colors = ["🟢", "🟡", "🔴"]

    st.subheader("📊 Modèle 1 : Risque de malignité")
    st.markdown(f"### {risk_colors[risk_pred]} Risque **{risk_labels[risk_pred]}**")
    c1, c2, c3 = st.columns(3)
    c1.metric("P(Faible)", f"{risk_proba[0]:.1%}")
    c2.metric("P(Intermédiaire)", f"{risk_proba[1]:.1%}")
    c3.metric("P(Élevé)", f"{risk_proba[2]:.1%}")
    ...
    if uploaded_file:
        ...
        if cancer_proba > 0.5:
            st.error(f"🔴 Cancer **PROBABLE** (probabilité : {cancer_proba:.1%})")
        else:
            st.success(f"🟢 Cancer **NON PROBABLE** (probabilité : {cancer_proba:.1%})")
        st.progress(float(cancer_proba))
    else:
        st.warning("⚠️ Chargez une radio pour le Modèle 2.")
'''
